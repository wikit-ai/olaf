...
class TestTokenSelectorDataPreprocessingParameters:
    ...
    def test_token_selector_attribute(self, selection_function):
        token_selector = TokenSelectorDataPreprocessing(
            selector=selection_function,
            token_sequence_doc_attribute="custom_selected_tokens",
        )
...
class TestTokenSelectorFunctions:
    def test_num_selector_true(self, example_sentence, example_sentence_no_num):
        token_selector = TokenSelectorDataPreprocessing(
            is_not_num, token_sequence_doc_attribute="custom_selected_tokens"
        )
    ...
    def test_punct_selector_true(self, example_sentence, example_sentence_no_punct):
        token_selector = TokenSelectorDataPreprocessing(
            is_not_punct, token_sequence_doc_attribute="custom_selected_tokens"
        )
        selected_tokens = token_selector._select_tokens([example_sentence[:]])
    ...
    def test_stop_word_selector_true(
        self, example_sentence, example_sentence_no_stop_word
    ):
        token_selector = TokenSelectorDataPreprocessing(
            is_not_stopword, token_sequence_doc_attribute="custom_selected_tokens"
        )
        selected_tokens = token_selector._select_tokens([example_sentence[:]])
    ...
    def test_url_selector_true(self, example_sentence, example_sentence_no_url):
        token_selector = TokenSelectorDataPreprocessing(
            is_not_url, token_sequence_doc_attribute="custom_selected_tokens"
        )
        selected_tokens = token_selector._select_tokens([example_sentence[:]])
...
class TestTokenSelectorRun:
    def test_run_without_doc_attribute(self, pipeline, example_sentence_no_num):
        ...
        selected_tokens = token_selector.corpus[0]._.get(
            token_selector._token_sequence_doc_attribute
        )
        assert len(selected_tokens) == len(example_sentence_no_num)
        assert all(
            [
    ...
    def test_run_selected_tokens_doc_attribute(
        self, pipeline_with_doc_attribute, example_sentence_no_num_url
    ):
        token_selector = TokenSelectorDataPreprocessing(
            is_not_num, token_sequence_doc_attribute="custom_selected_tokens"
        )
