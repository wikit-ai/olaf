...
from olaf.pipeline.pipeline_component.concept_relation_extraction.synonym_concept_extraction import SynonymConceptExtraction
...
def corpus(spacy_model):
    text_corpus = [
        "I like bike.",
        "I love beer.",
        "I eat pasta and pizza."
    ]
    corpus = list(spacy_model.pipe(text_corpus))
    return corpus

@pytest.fixture(scope="function")
def pipeline(spacy_model, corpus) -> Pipeline:
    pipeline = Pipeline(
...
@pytest.fixture(scope="function")
def empty_pipeline(spacy_model, corpus) -> Pipeline:
    empty_pipeline = Pipeline(spacy_model=spacy_model, corpus=corpus)
    return empty_pipeline

@pytest.fixture(scope="function")
def extraction_pipeline(spacy_model, corpus, preprocessing, component) -> Pipeline:
    extraction_pipeline = Pipeline(spacy_model=spacy_model, corpus=corpus)
...
@pytest.fixture(scope="function")
def concept_pipeline(spacy_model, corpus, preprocessing, component, concept_component) -> Pipeline:
    concept_pipeline = Pipeline(spacy_model=spacy_model, corpus=corpus)
...
def preprocessing() -> TokenSelectorDataPreprocessing:
    preprocessing = TokenSelectorDataPreprocessing(
        selector = is_not_stopword
    )
...
def test_pipeline_initialisation_error(spacy_model) -> None:
    with pytest.raises(PipelineCorpusInitialisationError):
         pipeline = Pipeline(spacy_model=spacy_model)
