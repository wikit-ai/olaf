...
if not spacy.tokens.doc.Doc.has_extension(custom_token_sequence_doc_attribute):
    spacy.tokens.doc.Doc.set_extension(custom_token_sequence_doc_attribute, default=[])
...
@pytest.fixture(scope="session")
def example_corpus(nlp_model) -> List[spacy.tokens.doc.Doc]:
    corpus_texts = [
    ...
    corpus = [
        nlp_model(text) for text in corpus_texts
    ]
...
@pytest.fixture(scope="session")
def example_pipeline(nlp_model, example_corpus) -> Pipeline:
...
def example_params() -> Dict[str, Any]:
    parameters = {
        "token_sequence_doc_attribute": custom_token_sequence_doc_attribute
    }
    return parameters
...
@pytest.fixture(scope="session")
def custom_tfidf_term_extraction(example_params, example_options) -> TFIDFTermExtraction :
    term_extraction = TFIDFTermExtraction (
        token_sequence_preprocessing= lambda span: [token.lower_.strip() for token in span],
...
class TestParamsOptionsDefaults:
    ...
    def test_default_token_sequences_doc_attribute(self, default_tfidf_term_extraction) -> None:
        assert default_tfidf_term_extraction._token_sequences_doc_attribute is None

    def test_default_token_sequence_preprocessing(self, default_tfidf_term_extraction) -> None:
    ...
    def test_default_candidate_term_threshold(self, default_tfidf_term_extraction) -> None:
...
class TestCustomParamsOptions:
    ...
    def test_custom_token_sequences_doc_attribute(self, custom_tfidf_term_extraction) -> None:
        assert custom_tfidf_term_extraction._token_sequences_doc_attribute == "custom_token_sequence_doc_attribute"
    ...
    def test_custom_candidate_term_threshold(self, custom_tfidf_term_extraction) -> None:
...
class TestTFIDFprocess:
    ...
    def test_extract_token_sequences(self, custom_tfidf_term_extraction, small_corpus) -> None:

        token_sequences = custom_tfidf_term_extraction._extract_token_sequences(corpus=small_corpus)
    ...
    def test_create_ngram_spans(self, custom_tfidf_term_extraction, small_corpus)-> None:

        token_sequences = custom_tfidf_term_extraction._extract_token_sequences(corpus=small_corpus)

        vocabulary_spans = custom_tfidf_term_extraction._create_ngram_spans(token_sequences)
        ...
        expected_texts = {
            ...
            'the second', 'second second', 'second document', 'second'
        }

        assert vocabulary_spans_texts == expected_texts

    @pytest.mark.filterwarnings("ignore::UserWarning")
    def test_spaced_term_corpus_occ_map(self, custom_tfidf_term_extraction, example_pipeline) -> None:
        ...
        token_sequences = custom_tfidf_term_extraction._extract_token_sequences(corpus=example_pipeline.corpus)
        vocabulary_spans = custom_tfidf_term_extraction._create_ngram_spans(token_sequences)
        spaced_term_corpus_occ_map = custom_tfidf_term_extraction._spaced_term_corpus_occ_map(vocabulary_spans)
    ...
    @pytest.mark.filterwarnings("ignore::UserWarning")
    def test_tfidf_run(self, example_params, example_options, nlp_model, example_corpus) -> None:
