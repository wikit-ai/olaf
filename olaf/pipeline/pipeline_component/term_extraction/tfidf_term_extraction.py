...
class TFIDFTermExtraction(TermExtractionPipelineComponent):
    ...
    def __init__(
        ...
        self.tfidf_agg_type = None
        self.candidate_term_threshold = None
    ...
    def _check_options(self) -> None:
        """Check wether the required tunable options are correctly provided and if not
        sets the default values.

        For the TFIDFtermExtraction pipeline component the options to check are:
        - "threshold": the TF-IDF score threshold below which terms will be ignored.
        - "max_term_token_length": the maximum number of tokens a candidate term can have.
        """

        self.candidate_term_threshold = self.options.get("threshold")
        self._max_term_token_length = self.options.get("max_term_token_length")
        self.tfidf_agg_type = self.options.get("tfidf_agg_type")
    ...
    def _spaced_term_corpus_occ_map(
        self, token_seqs_spans: List[spacy.tokens.Span]
        ...
        for span in token_seqs_spans:
            ...
            term_corpus_occ_mapping[spaced_term].append(span)
